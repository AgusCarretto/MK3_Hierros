...
| feat-006 | La grilla de "Nuestros Trabajos" carga miniaturas livianas (no la foto original) por card; el detalle/carrusel sigue mostrando resolución completa | `cd mk3_hierros_back && npm run build && cd ../mk3_hierros_front && npm run build` | passing | exec-plan perf-002 | Ver `docs/exec-plans/completed/2026-08-17_perf-002_works-image-thumbnails.md`. Validado end-to-end contra Postgres de producción: -85.3% de payload, sin regresión visual. |
